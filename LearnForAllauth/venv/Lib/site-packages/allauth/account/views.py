...
class PasswordChangeFromKeyView(AjaxCapableProcessFormViewMixin, FormView):
    template_name = (
        "account/password_change_from_key." + app_settings.TEMPLATE_EXTENSION)
    form_class = ChangePasswordKeyForm
    success_url = reverse_lazy("account_change_password_from_key_done")

    def get_form_class(self):
        return get_form_class(app_settings.FORMS,
                              'change_password_from_key',
                              self.form_class)
    ...
    def dispatch(self, request, uidb36, key, **kwargs):
        ...
        if self.key == INTERNAL_RESET_URL_KEY:
            ...
            if token_form.is_valid():
                ...
                return super(PasswordChangeFromKeyView, self).dispatch(request,
                                                                      uidb36,
                                                                      self.key,
                                                                      **kwargs)
        else:
    ...
    def get_context_data(self, **kwargs):
        ret = super(PasswordChangeFromKeyView, self).get_context_data(**kwargs)
        ret['action_url'] = reverse(
            'account_change_password_from_key',
            kwargs={'uidb36': self.kwargs['uidb36'],
                    'key': self.kwargs['key']})
        return ret

    def get_form_kwargs(self):
        kwargs = super(PasswordChangeFromKeyView, self).get_form_kwargs()
    ...
    def form_valid(self, form):
        ...
        return super(PasswordChangeFromKeyView, self).form_valid(form)


password_change_from_key = PasswordChangeFromKeyView.as_view()

class PasswordChangeFromKeyDoneView(TemplateView):
    template_name = (
        "account/password_change_from_key_done." +
        app_settings.TEMPLATE_EXTENSION)


password_change_from_key_done = PasswordChangeFromKeyDoneView.as_view()


class PasswordChangeDoneView(TemplateView):
    template_name = (
        "account/password_change_done." + app_settings.TEMPLATE_EXTENSION)
